...
The paragraph above assumes canon exists somewhere. Often it does not: a fresh chat, a bare code session, a repo with no show files in it. That case has a specific failure mode and it is quiet.

With no canon, nothing errors. The method still runs, fills the gaps with plausible defaults, and returns work that is well-formed and generic. It reads like the skill underperforming when what actually happened is that the skill was starving. This is the single most common reason output from a bare session comes back worse than output from a project that has canon attached, and the difference is the files, not the model.

So before drafting anything show-specific, confirm four things are actually available:

1. Runtime and word count per episode.
2. Episode count, free-block size, and where the gate sits.
3. The lead, the opponent who holds the rulebook, and the season question as one yes-or-no.
4. Locked character names and spellings.

Have them: work. Missing some: name exactly which, and either ask, or propose values and mark every one `PROPOSED` until confirmed. Do not quietly pick a runtime and do not invent a character name. A scene built on a guessed format lock has to be thrown away, and the rewrite costs more than the question would have.

Offer to write the confirmed answers somewhere persistent, so the next session does not re-derive them and possibly answer differently.

Feedback, analysis, ideation, hook scoring and research are exempt. They do not need canon and gating them behind it is just friction.

### Which skill owns what

This skill has two companions and they overlap on purpose. Run them together. When more than one is active:

- **vertical-drama-muse** owns invention and the line: premises, twists, titles, voice, rebuilding a flat scene.
- **vertical-drama-showrunner**, this skill, owns structure and money: gates, retention, paywall economics, pipeline discipline, lock points.
- **vertical-script-editor** owns the verdict: audits, coverage, hook scoring and the episode grid, judged against its fixed rule set rather than taste.

Where they disagree, the editor's rule set wins on an audit judgment, this skill wins on structure and format, the muse wins on the line itself. Name which one you are following when it is not obvious. Canon still beats all three.
